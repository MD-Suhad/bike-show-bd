...
const OrderDetails = () => {
  ...
  const singleItemsData = carsDetails.filter((data) => parseInt(data.id) === parseInt(productsId));
  ...
  const handleOrderSubmit = e => {
    console.log('ordered',order);
        // Send data to server
        fetch('https://pure-stream-69695.herokuapp.com/placeOrders', {
          method: 'POST',
          headers: {
    ...
    <Container className="my-5 d-flex justify-content-between ">
      <div>
        <Card className="p-3 form" style={{ width: "20rem" }}>
          <Card.Img variant="top" src={singleItemsData[0]?.image} />
          <Card.Body>
